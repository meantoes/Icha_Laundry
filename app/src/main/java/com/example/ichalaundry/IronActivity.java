...
import com.example.ichalaundry.R;
import com.example.ichalaundry.FunctionHelper;
import com.example.ichalaundry.AddDataViewModel;
...
public class IronActivity extends AppCompatActivity {
    ...
    private void setDataSprei() {
        ...
        imageAdd4.setOnClickListener(v -> Toast.makeText(IronActivity.this, "Layanan tidak tersedia untuk sprei!", Toast.LENGTH_SHORT).show());

        imageMinus4.setOnClickListener(v -> Toast.makeText(IronActivity.this, "Layanan tidak tersedia untuk sprei!", Toast.LENGTH_SHORT).show());
    }

    private void setDataKarpet() {
        tvKarpet.setText(FunctionHelper.rupiahFormat(hargaKarpet));
        imageAdd5.setOnClickListener(v -> Toast.makeText(IronActivity.this, "Layanan tidak tersedia untuk karpet!", Toast.LENGTH_SHORT).show());

        imageMinus5.setOnClickListener(v -> Toast.makeText(IronActivity.this, "Layanan tidak tersedia untuk karpet!", Toast.LENGTH_SHORT).show());
    }
    ...
    private void setInputData() {
        btnCheckout.setOnClickListener(v -> {
            if (totalItems == 0 || totalPrice == 0) {
                Toast.makeText(IronActivity.this, "Harap pilih jenis barang!", Toast.LENGTH_SHORT).show();
            } else {
                addDataViewModel.addDataLaundry(strTitle, totalItems, strCurrentLocation, totalPrice);
                Toast.makeText(IronActivity.this, "Pesanan Anda sedang diproses, cek di menu riwayat", Toast.LENGTH_SHORT).show();
                finish();
            }
        });
    }
